...
const messages = {
  ...
  async getFilterValues() {
    // Fetch unique values for category, property_type, and purpose
    const { data: categories } = await supabase.from('messages').select('category');
    const { data: propertyTypes } = await supabase.from('messages').select('property_type');
    const { data: purposes } = await supabase.from('messages').select('purpose');

    return {
      categories: [...new Set(categories?.map(c => c.category) || [])].filter(Boolean),
      propertyTypes: [...new Set(propertyTypes?.map(p => p.property_type) || [])].filter(Boolean),
      purposes: [...new Set(purposes?.map(p => p.purpose) || [])].filter(Boolean)
    };
  }
};
