...
function createDefaultDb() {
  ...
  return {
    users: [adminUser, legacyAdminUser, brokerUser],
    messages,
    regions,
    reset_requests: []
  };
...
const messages = {
  ...
  async getAll(filters = {}, limit = 100, offset = 0) {
    const db = loadDb();
    const now = new Date();
    const allFiltered = sortByDateDesc(filterMessages(db.messages, filters), 'date_of_creation');
    const notFutureDated = allFiltered.filter(m => !m.date_of_creation || new Date(m.date_of_creation) <= now);
    return notFutureDated.slice(offset, offset + limit);
  },
...
async function getStats() {
  ...
  const totalSenders = new Set(
    db.messages
      .map((message) => {
        const mobile = String(message.sender_mobile || '').trim();
        const name = String(message.sender_name || '').trim().toLowerCase();
